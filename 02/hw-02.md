...
[`preemptible`](https://cloud.yandex.ru/docs/compute/concepts/preemptible-vm) - прерываемость виртуальной машины. В случае простоя в течение 24 часов или при нехватке ресурсов для запуска обычной виртуальной машины прерываемая виртуальная машина остановится. Для лабораторных работ, где не требуется постоянная доступность виртуальной машины, этот параметр экономит средства
