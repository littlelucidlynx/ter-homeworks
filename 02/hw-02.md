...
2. В Yandex Cloud создан сервисный аккаунт с ролью **editor** и ключ
...
Ошибка в наименовании платформы. Необходимо выбрать одну из стандартных платформ, представленных в описании сервиса:
...
8. ```preemptible = true``` - прерываемость виртуальной машины. В случае простоя в течение 24 часов виртуальная машина выключится. Для лабораторных работ, где не требуется постоянная доступность виртуальной машины, этот параметр экономит средства

```core_fraction=5``` - гарантированная доступность ядер vCPU. Виртуальная машина стоит дешевле, но при больших нагрузках на процессор может подвисать
