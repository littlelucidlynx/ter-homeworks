...
[`preemptible`](https://cloud.yandex.ru/docs/compute/concepts/preemptible-vm) - прерываемость виртуальной машины. В случае простоя в течение 24 часов виртуальная машина выключится. Для лабораторных работ, где не требуется постоянная доступность виртуальной машины, этот параметр экономит средства

[`core_fraction`](https://cloud.yandex.ru/docs/compute/concepts/performance-levels) - гарантированная доступность ядер vCPU. Виртуальная машина стоит дешевле, но при больших нагрузках на процессор может подвисать
